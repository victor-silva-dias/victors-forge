...
const PricingPage = () => {
    ...
    const handlePlanClick = (planKey, cycle) => {
        ...
        if (plan?.isEnterprise) {
            ...
            mixpanel.track(`Click to Checkout - ${plan.name} ${cycleInPortuguese}`, {
                'Plan Name': plan.name,
                'Billing Cycle': cycle
            });
            ...
            <main className="flex-grow container mx-auto px-6 py-12 md:py-20">
                <div className="text-center mb-10">
                    <h1 className="text-3xl md:text-4xl font-extrabold mb-4">
                        Conhecimento e Execução. <span className="text-rocketBlue">Tudo em um só lugar.</span>
                    </h1>
                    <p className={clsx('text-md md:text-lg max-w-3xl mx-auto mb-4', isDarkMode ? 'text-gray-300' : 'text-gray-600')}>
                        Todos os planos garantem acesso ilimitado à <strong>Apolus Academy</strong> e a todas as nossas <strong>ferramentas de IA</strong>. Escolha seu plano com base no volume de uso que sua advocacia precisa.
                    </p>
                ...
                <div className={clsx('text-center p-8 rounded-lg my-16', isDarkMode ? 'bg-gray-800' : 'bg-gray-100')}>
                    <h2 className={clsx('text-3xl font-bold mb-8', isDarkMode ? 'text-white' : 'text-gray-900')}>Uma plataforma, dois pilares</h2>
                    <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                        <div className="flex flex-col items-center">
                            <FaBrain className="text-5xl text-rocketBlue mb-4"/>
                            <h3 className="text-xl font-semibold mb-2">Conhecimento Ilimitado</h3>
                            <p className={isDarkMode ? 'text-gray-300' : 'text-gray-700'}>
                                Domine o método da Inteligência Jurídica Expandida com acesso total à <strong>Apolus Academy</strong>.
                            </p>
                        </div>
                        <div className="flex flex-col items-center">
                            <FaBolt className="text-5xl text-rocketBlue mb-4"/>
                            <h3 className="text-xl font-semibold mb-2">Execução que Escala</h3>
                            <p className={isDarkMode ? 'text-gray-300' : 'text-gray-700'}>
                                Use nossas <strong>IAs especialistas</strong> para criar teses e estratégias no volume que sua advocacia precisa.
                            </p>
                        </div>
                    </div>
                </div>
